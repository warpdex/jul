...
const blake2b160 = require('./hashes/blake2b160');
const blake2b256 = require('./hashes/blake2b256');
const hash160 = require('./hashes/hash160');
const hash256 = require('./hashes/hash256');
const keccak256 = require('./hashes/keccak256');
const ripemd160 = require('./hashes/ripemd160');
const sha256 = require('./hashes/sha256');

/*
 * Constants
 */

const SUPPORT_FILE = path.resolve(__dirname, '..', 'etc', 'support.yul');
const BUILTINS_FILE = path.resolve(__dirname, '..', 'etc', 'builtins.yul');
...
class Parser {
  ...
  StructDefinition() {
    ...
    const memory = this.keyword('memory');
  ...
  Macro() {
    ...
    if (this.consume(':=')) {
      const expr = this.assert(this.Expression(true));
  ...
  VariableDeclaration() {
    ...
    if (this.consume(':='))
      expr = this.assert(this.Expression(true));
  ...
  TypedIdentifierList() {
    ...
    do {
      const prefix = this.match(/^[&*](?=[a-zA-Z_$])/);
      const name = this.Identifier();

      if (!name) {
        this.assert(prefix === null);
      ...
      if (prefix)
        storage = prefix === '&' ? CALLDATA : MEMORY;

      if (!this.peek(':=') && this.consume(':'))
        type = this.assert(this.TypeName(name));

      items.push([name, type, storage]);
    } while (this.consume(','));
  ...
  Assignment() {
    ...
    const rhs = this.assert(this.Expression(true));
  ...
  Expression(inferType = false) {
    // Expression =
    //     FunctionCall | Identifier | Literal
    // Makes more sense to put Identifier last.
    return this.StructInitializerLegacy()
        || this.ParenInitializer(inferType)
        || this.BraceInitializer(inferType)
        ...
        || this.CallDataReference();
  }

  StructInitializerLegacy() {
    // StructInitializerLegacy =
    ...
    return {
      type: 'StructInitializerLegacy',
      name,
      args
    };
  }

  ParenInitializer(inferType = false) {
    // ParenInitializer = ( Identifier ':' ) ?
    ...
    const ptr = this.Expression();

    // Note: we could do something like:
    //
    //   if (ptr && !this.consume(','))
    //
    // However, requiring the pointer with braces
    // frees us up to implement statement expressions
    // in the future. e.g. `let z := ({ add(x, y) })`.
    if (!ptr || !this.consume(',')) {
      this.restore(saved);
      return null;
    }

    const [args, dots] = this.StructFields();
  ...
  BraceInitializer(inferType = false) {
    // BraceInitializer = ( Identifier ':' )? StructFields
    const saved = this.save();
    const name = this.Identifier();
  ...
  StructFields() {
    // StructAssignment = ( '.' Identifier ( ':=' Expression ) ? ) | Expression
    // StructFields = '{' StructAssignment ( ',' StructAssignment )* )? '}'
    if (!this.consume('{'))
      return [null, false];
    ...
    if (!this.peek('}')) {
      do {
        ...
        if (dots) {
          ...
          lhs = this.assert(this.Identifier());
        ...
        let rhs = this.DefaultIdentifier();

        if (!rhs)
          rhs = this.assert(this.Expression());
  ...
  FunctionCall() {
    ...
    if (!this.peek(')')) {
      do {
        args.push(this.assert(this.Expression(true)));
      } while (this.consume(','));
    }
  ...
  CallDataReference() {
    // CallDataReference = '&calldata'
    if (!this.keyword('&calldata'))
      return null;
...
class Scope {
  ...
  *scopes() {
  ...
  findHoisted(name) {
    for (const scope of this.scopes()) {
      const result = scope.hoisted.get(name);
  ...
  findMember(node) {
    ...
    let obj = this.findVar(name);

    if (obj == null)
      throw new Error(`Referencing non-existent variable: ${name}`);

    if (obj.type == null)
      throw new Error(`Member access on undefined type: ${name}`);
    ...
    if (struct == null)
      throw new Error(`Struct definition not found: ${name}:${type}`);
  ...
  addVar(name, type, storage) {
    if (this.vars.has(name.value))
      throw new Error(`Variable already declared: ${name.value}`);

    if (this.consts.has(name.value))
      throw new Error(`Constant already defined: ${name.value}`);
...
class Transformer {
  constructor(root, options, hasher = null) {
    ...
    if (options != null) {
      ...
      if (options.warnFunc != null)
        warnFunc = options.warnFunc;
      ...
      assert(typeof warnFunc === 'function');
    }
  ...
  _throw(node, msg, ...args) {
    assert(node && typeof node.type === 'string');
    assert(typeof msg === 'string');
    throw new Error(format(msg, args) + this._info(node));
  }

  _warn(node, msg, ...args) {
    assert(node && typeof node.type === 'string');
    assert(typeof msg === 'string');
    this.warnFunc(format(msg, args) + this._info(node));
  }

  addMemStruct(struct, scope) {
    scope.addStruct(struct);
    scope.funcs.set(struct.fname, struct.generate());
  }

  addMethodStruct(method, scope) {
    if (method.params.length > 0)
      this.addMemStruct(MemStruct.fromMethod(method), scope);

    if (method.returns.length > 0)
      this.addMemStruct(MemStruct.fromReturns(method), scope);
  }
  ...
  _hoist(block, scope) {
    ...
    this.scope = scope.child(scope.node);
    this.scope.depth -= 1;
    ...
    for (const node of block.body) {
      switch (node.type) {
        ...
        case 'FunctionCall': {
          ...
          if (name.value === 'undefine' &&
              node.args.length === 1 &&
              node.args[0].type === 'Identifier') {
            this.scope.undefMacro(node.args[0].value);
          }
        ...
        case 'FunctionDefinition': {
          ...
          scope.hoisted.set(name.value, {
            type: node.type,
            name,
            params: this.paramsRewrite(node.params),
            modifier: node.modifier,
            returns: this.paramsRewrite(node.returns),
            block: node.block,
  ...
  params(node, scope) {
    ...
    for (const [ident, kind, storage] of node.items) {
      const name = this.rewrite(ident);
      const type = kind ? this.rewrite(kind) : null;

      scope.addVar(name, type, storage);

      if (type) {
        const struct = scope.findStruct(type.value);

        if (!struct) {
          items.push([name, type, 0]);
          continue;
        }

        if (struct.isMemory !== (storage !== STACK)) {
          const id = `${ident.value}:${kind.value}`;
          throw new Error(`Invalid storage class for ${id}`);
        }
      }

      items.push([name, null, 0]);
    }
  ...
  rewrite(node) {
    ...
    const result = this._rewrite(node);
    this.parents.pop();
    return result;
  }
  ...
  _rewrite(node) {
    ...
    switch (node.type) {
      ...
      case 'StructDefinition': {
        if (node.memory) {
          const struct = new MemStruct(this.prepare(node));
      ...
      case 'Interface': {
        ...
        for (const method of iface.methods) {
          ...
          this.addMethodStruct(method, scope);
        }
      ...
      case 'ConstructorDefinition': {
        if (scope.node.type !== 'CodeBlock')
          throw new Error('Constructor must be inside a code block.');

        if (scope.parent.node.type !== 'ObjectBlock')
          throw new Error('Code block must be inside an object block.');

        scope.ctor = true;

        // Could choose to not generate function and simply
        // save node on scope.ctor, generate code on construct().
        const object = scope.parent.node;
        const ctor = new Constructor(this.prepare(node), object.name);
        const child = scope.child(node);
        const check = ctor.check();

        ctor.addDeps(child); // Could be ctor.addDeps(scope) and move elsewhere.
        ...
        const size = Identifier('__size');
        const params = [[off, null], [size, null]];
        const immutables = [];

        for (const [name, value] of child.immutable)
          immutables.push(SetImmutable(Literal(0), name, value));

        return {
          type: 'FunctionDefinition',
          name: Identifier('__construct'),
          ...
          block: Block([
            ...check.nodes,
            ...block.body,
            DataCopy(Literal(0), off, size),
            ...immutables,
            Return(Literal(0), size)
          ]),
          builtin: false
        };
      }
      ...
      case 'EventDeclaration': {
        ...
        const count = event.params.reduce((acc, param) => {
          return acc + Number(!param.indexed);
        }, 0);

        if (count > 0)
          this.addMemStruct(MemStruct.fromEvent(event), scope);
      ...
      case 'ErrorDeclaration': {
        ...
        if (error.params.length > 0)
          this.addMemStruct(MemStruct.fromError(error), scope);
      ...
      case 'MemberAssignment': {
        ...
        const [mem, obj] = scope.findMember(lhs);

        assert(lhs.name.value === obj.name.value);

        if (node.or)
          return mem.put(obj, rhs);

        return mem.write(obj, rhs);
      }
      ...
      case 'EventSignature': {
        ...
        if (node.params) {
          ...
          const signature = eventHash(name, types, node.packed);
      ...
      case 'StructInitializerLegacy': {
        ...
        if (struct.isMemory)
          throw new Error(`Cannot use legacy struct initializer: ${name}`);
        ...
        return struct.fold(args, null);
      }

      case 'StructInitializer': {
        let nodeName = node.name;

        if (!nodeName && this.parents.length > 1) {
          const parent = this.parents[this.parents.length - 2];

          switch (parent.type) {
            case 'VariableDeclaration': {
              if (parent.vars.items.length === 1) {
                const type = parent.vars.items[0][1];
                if (type)
                  nodeName = this.rewrite(type);
              }
              break;
            }

            case 'Assignment': {
              if (parent.lhs.items.length === 1) {
                const name = this.value(parent.lhs.items[0]);
                const obj = scope.findVar(name);
                nodeName = obj ? obj.type : null;
              }
              break;
            }
            ...
            case 'FunctionCall': {
              ...
              const name = this.value(parent.name);
              const func = scope.findHoisted(name);
              if (func && index < func.params.items.length)
                nodeName = func.params.items[index][1];
              break;
            }
          }
        }
        ...
        if (!nodeName)
          throw new Error('No type for struct initializer');

        const name = this.value(nodeName);
        const struct = scope.findStruct(name);

        if (struct == null)
          throw new Error(`Unknown struct: ${name}`);

        if (struct.isMemory && !node.ptr)
          throw new Error(`Must past pointer for struct: ${name}`);

        if (!struct.isMemory && node.ptr)
          throw new Error(`Must not past pointer for struct: ${name}`);

        if (struct.isMemory)
          scope.dependsOn(struct.fname);
        ...
        if (node.ptr)
          ptr = this.rewrite(node.ptr);

        for (const [lhs, rhs] of node.args) {
          if (lhs)
            args.push([this.rewrite(lhs), this.rewrite(rhs)]);
          else
            args.push([null, this.rewrite(rhs)]);
        }

        return struct.fold(struct.flatten(args), ptr);
      }
      ...
      case 'FunctionCall': {
        ...
        if (macro && macro.type === 'MacroDefinition') {
          if (node.args.length !== macro.params.length)
            this._throw(node, 'Invalid arguments for macro: %s', macro.name);
        ...
        const func = scope.findHoisted(name.value)
                  || scope.findFunc(name.value);

        if (func && node.args.length !== func.params.items.length)
          this._throw(node, 'Invalid arguments for call: %s', name.value);

        if (func && this.parents.length > 1) {
          const parent = this.parents[this.parents.length - 2];

          switch (parent.type) {
            case 'VariableDeclaration': {
              if (parent.vars.items.length !== func.returns.items.length) {
                this._throw(node, 'Invalid return parameters for call: %s',
                                  name.value);
              }
              break;
            }
            case 'Assignment': {
              if (parent.lhs.items.length !== func.returns.items.length) {
                this._throw(node, 'Invalid return parameters for call: %s',
                                  name.value);
              }
              break;
      ...
      case 'MemberIdentifier': {
        ...
        const [mem, obj] = scope.findMember(id);

        assert(id.name.value === obj.name.value);

        if (id.ref) {
          if (obj.storage === STACK)
            throw new Error('Cannot use dereference stack struct member.');

          if (mem.offsetof() === 0)
            return obj.name;

          return Add(obj.name, Literal(mem.offsetof()));
        }
  ...
  builtin(name, node, scope) {
    ...
    switch (name.value) {
      ...
      case 'construct': {
        if (scope.node.type !== 'CodeBlock')
          throw new Error('construct() must be inside a code block.');
        ...
        if (!scope.ctor) {
          const object = Literal(scope.parent.node.name);

          return Root([
            RequireZero(CallValue()),
            RequireEq(CodeSize(), DataSize(object)),
            DataCopy(Literal(0), DataOffset(name), DataSize(name)),
            Return(Literal(0), DataSize(name))
          ]);
        }

        return Call('__construct', [DataOffset(name), DataSize(name)]);
      }
      ...
      case 'bitsof': {
        ...
        if (node.args[0].type === 'MemberIdentifier') {
          ...
          if (name.value === 'sizeof')
            return Literal(mem.sizeof());

          return Literal(mem.bitsof());
        }
        ...
        if (arg.type === 'Identifier') {
          ...
          if (name.value === 'sizeof')
            return Literal(struct.sizeof());

          return Literal(struct.bitsof());
        }
    ...
    switch (node.type) {
      ...
      case 'TypedIdentifierList': {
        ...
        for (const [ident, kind, storage] of node.items) {
          ...
          if (type && storage) {
            const prefix = storage === CALLDATA ? '@' : '*';
            idents.push(prefix + [name, type].join(':'));
          } else if (type) {
      ...
      case 'StructInitializer': {
        ...
        if (node.args.length > 0) {
          ...
          if (node.dots) {
            for (const [lhs, rhs] of node.args)
              args.push(`  .${this.encode(lhs)} := ${this.encode(rhs)}`);
  ...
  encode(node) {
    ...
    switch (node.type) {
      ...
      case 'TypedIdentifierList': {
        return this.rewrite({
          type: 'TypedIdentifierList',
          items: node.items.map(([name, type, storage]) => {
            return [this.encode(name), this.encode(type), storage];
          })
        });
      }
      ...
      case 'StructInitializer': {
        return this.rewrite({
          type: 'StructInitializer',
          name: this.encode(node.name),
          ptr: this.encode(node.ptr),
          dots: node.dots,
          args: node.args.map(([lhs, rhs]) => {
            return [this.encode(lhs), this.encode(rhs)];
          })
        });
      }
...
class Struct {
  ...
  init(node) {
    ...
    for (const member of node.members) {
      ...
      if (width === 0 || width > 256 || (signed && (width & 7)))
        throw new Error(`Invalid integer width: ${id} (${type})`);
      ...
      const default_ = member.value;
      const mem = new StructMember(name, offset, width, signed, default_);

      n |= mem.value << BigInt(mem.shift);

      if (name === '+') {
        padding |= mem.value << BigInt(mem.shift);
      } else {
  ...
  flatten(args) {
    ...
    const dots = args[0][0] != null;

    if (!dots)
      return args.map(([, rhs]) => rhs);
    ...
    for (const [lhs, rhs] of args) {
      const mem = this.map.get(lhs.value);

      if (!mem)
        throw new Error(`Unknown member name: ${this.name}->${lhs.value}`);

      out[this.members.indexOf(mem)] = rhs;
    }
  ...
  fold(args, ptr = null) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (!arg || isDefaultIdentifier(arg)) {
        n |= mem.value << BigInt(mem.shift);
        continue;
      }

      if (isLiteral(arg)) {
        let value = toBigInt(arg.value);

        if (mem.signed)
          value &= mem.mod;

        if (value > mem.mod)
          throw new Error(`Literal exceeds width for member ${mem.name}`);

        n |= value << BigInt(mem.shift);
...
class StructMember {
  constructor(name, offset, width, signed, default_) {
    const shift = 256 - (offset + width);
    const mask = (1n << BigInt(width)) - 1n;
    const imask = (mask << BigInt(shift)) ^ U256_MAX;

    let value = toBigInt(default_.value);

    if (signed)
      value &= mask;

    if (value > mask)
      throw new Error(`Literal exceeds width for member ${name}`);
  ...
  read({name}) {
    const expr = this._read(name);

    if (this.signed && this.width < 256) {
      const b = (this.width / 8) - 1;
      return Call('signextend', [Literal(b), expr]);
    }
  ...
  put({name}, value) {
    ...
    if (isLiteral(value)) {
      ...
      if (num > this.mod)
        throw new Error(`Literal exceeds width for member ${this.name}`);
...
class MemStruct {
  constructor(node) {
    ...
    this.width = 0;
    this.isMemory = true;
    this.packed = Boolean(node.packed);
    this.fname = `__istruct_${this.name}`;
    this.init(node);
  }
  ...
  init(node) {
    ...
    for (const member of node.members) {
      ...
      const name = member.name.value;
      const id = `${this.name}->${name}`;
      const abi = new Parser(type).ABIType();
      const width = abi.width || 256;
      const size = (width + 7) >>> 3;

      if (this.packed && abi.array)
        throw new Error(`Cannot handle array type: ${id}`);
      ...
      const default_ = member.value;
      const mem = new MemStructMember(name,
                                      abi,
                                      offset,
                                      packed,
                                      default_);
      ...
      if (name === '+') {
        this.padding.push(mem);
      } else {
        this.members.push(mem);
        this.map.set(name, mem);
      }

      offset += this.packed ? size : 32;
    }
  ...
  generate() {
    const pads = this.padding;
    const ptr = Identifier('__ptr');
    const ret = Identifier('__ret');
    const params = [[ptr, null, 0]];
    const rets = [[ret, null, 0]];
    const body = [];

    body.push(Assignment(ret, ptr));
    ...
    for (let i = 0; i < this.members.length; i++) {
      const mem = this.members[i];
      const arg = Identifier(dollar(mem.name) || `__arg${i}`);

      params.push([arg, null, 0]);

      while (p < pads.length && pads[p].offset < mem.offset) {
        const pad = pads[p++];
        store(pad, Literal(pad.value));
      }
  ...
  fold(args, ptr = null) {
    ...
    return Call(this.fname, params);
  }

  static fromParams(name, params, packed) {
    const members = [];
  ...
  static fromMethod(method) {
    return this.fromParams('abi.' + method.name, method.params, method.packed);
  }

  static fromReturns(method) {
    return this.fromParams('ret.' + method.name, method.returns, method.packed);
  }

  static fromEvent(event) {
    return this.fromParams('evt.' + event.name, event.params, event.packed);
  }

  static fromError(error) {
    return this.fromParams('err.' + error.name, error.params, error.packed);
  }
}

/*
 * Memory Struct Member
 */

class MemStructMember {
  constructor(name, abi, offset, packed, default_) {
    const width = (abi.width + 7) & ~7;
    const shift = 256 - width;
    const mask = (1n << BigInt(width)) - 1n;
    const imask = (mask << BigInt(shift)) ^ U256_MAX;

    let value = toBigInt(default_.value);

    if (packed && abi.base === 'int')
      value &= mask;
    ...
    this.base = abi.base;
    this.width = width;
    this.array = abi.array;
    this.nested = abi.nested;
    this.signed = abi.base === 'int';
    this.offset = offset;
    this.packed = packed;
    this.value = value;
    this.shift = shift;
  ...
  _ptr(obj) {
    return Add(obj.name, Literal(this.offset));
  }

  _load(obj) {
    const ptr = this._ptr(obj);

    if (obj.storage === CALLDATA)
      return Call('calldataload', [ptr]);
  ...
  _store(obj, expr) {
    ...
    if (this.packed && this.width === 8)
      return MemStore8(ptr, expr);

    return MemStore(ptr, expr);
  }

  read(obj) {
    const {shift} = this;
    const load = this._load(obj);

    if (this.packed && this.width < 256) {
      const expr = this.width === 8
                 ? Byte(0, load)
                 : Shr(shift, load);
    ...
    if (obj.storage === CALLDATA && this.array)
      return Add(load, Literal(4));

    if (this.isPadded())
      return Shr(shift, load);

    return load;
  }

  _write(obj, value, isPut) {
    const {shift} = this;
    ...
    if (this.packed) {
      if (isLiteral(value)) {
        ...
        if (this.width === 8 || this.width === 256)
          return this._store(obj, Literal(num));
        ...
        if (this.width === 8 || this.width === 256)
          return this._store(obj, value);

        value = Shl(shift, value);
      }

      if (isPut)
        return this._store(obj, value);

      const load = And(this._load(obj), this.imask);

      return this._store(obj, Or(value, load));
    }
  ...
  put(obj, value) {
    return this._write(obj, value, true);
  }

  write(obj, value) {
    return this._write(obj, value, false);
  }
...
class Constructor {
  ...
  generate(name, create2 = false) {
    ...
    const ptr = Identifier('__ptr');
    const args = [[pos, null, 0], [off, null, 0], [size, null, 0]];
    ...
    return {
      ...
      modifier: null,
      returns: TypedIdentifierList([[addr, null, 0]]),
      block: Block(body),
      builtin: false
    };
  }
  ...
  check() {
    ...
    if (this.params.length > 0) {
      ...
      if (this.array || this.modifier === 'lax')
  ...
  toSolidity() {
    ...
    return us(eol(this.comment)) + str.replace(/^/gm, '// ');
  }
}
...
class Method {
  ...
  generate(name, attempt = false) {
    ...
    const pos = Identifier('__pos');
    const addr = Identifier('__addr');
    const args = [[pos, null, 0], [addr, null, 0]];
  ...
  check() {
    ...
    if (!this.isFunction() || this.modifier === 'unchecked')
      return Root(nodes);
  ...
  toSolidity() {
    ...
    return us(eol(this.comment)) + str;
  }
}
...
class Event {
  ...
  init(node) {
    ...
    this.signature = eventHash(this.name, types, this.packed);
  }
...
function Let(ident, expr) {
  return VariableDeclaration(TypedIdentifierList([[ident, null, 0]]), expr);
}
...
function FunctionConstant(name, expr, memguard = false) {
  ...
  return {
    ...
    params: TypedIdentifierList(),
    modifier: null,
    returns: TypedIdentifierList([[ret, null, 0]]),
    block: Block(body),
    builtin: false
  };
}
...
function isExpression(node) {
  return node.type === 'StructInitializerLegacy'
      || node.type === 'StructInitializer'
...
function abiHash(name, types, packed = false) {
  const modifier = packed ? ' packed' : '';
  const canon = types.map(canonicalize);
  const preimage = `${name}(${canon.join(',')})${modifier}`;
  const raw = Buffer.from(preimage, 'binary');
  return keccak256.digest(raw);
}

function methodHash(name, types) {
  const hash = abiHash(name, types, false);
  return '0x' + hash.toString('hex', 0, 4);
}

function eventHash(name, types, packed) {
  const hash = abiHash(name, types, packed);
...
function us(str) {
  if (str)
    str = str.replace(/(@(?:param|return)[ \t]+)([a-z_])/g, '$1_$2');
...
function format(msg, args) {
  let i = 0;
  return String(msg).replace(/%[sd]/g, (type) => {
    if (i >= args.length)
      return type;
