...
class Parser {
  ...
  StructDefinition() {
    ...
    const memory = this.consume('memory');
    const packed = this.consume('packed');
  ...
  VariableDeclaration() {
    ...
    if (this.consume(':='))
      expr = this.assert(this.NakedStruct() || this.Expression());
  ...
  Assignment() {
    ...
    const rhs = this.assert(this.NakedStruct() || this.Expression());
  ...
  StructInitializer() {
    // StructInitializer = Identifier ? '(' ( Expression ',' )? StructFields ')'
    const saved = this.save();
    const name = this.Identifier();
