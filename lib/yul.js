...
class Parser {
  ...
  ABIType() {
    ...
    return {
      ...
      width,
      array,
      dynamic: array || width === 0,
      nested: array && width === 0
    };
  }
...
class Constructor {
  ...
  addDeps(scope) {
    if (this.modifier !== 'unchecked') {
      if (this.dynamic)
        scope.dependsOn('__check_memory_nested');
  ...
  generate(name, create2 = false) {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      if (param.nested)
        throw new Error('Cannot generated code for nested type.');
...
class Method {
  ...
  addDeps(scope) {
    if (this.dynamic)
      scope.dependsOn('__check_calldata_nested');
  ...
  generate(name, attempt = false) {
    ...
    if (this.isView()) {
      call = Call('staticcall', [
        ...
        pos,
        srclen,
        this.retDynamic ? Literal(0) : pos,
        this.retDynamic ? Literal(0) : dstlen
      ]);
    } else {
      call = Call('call', [
        Gas(),
...
class MethodParam {
  ...
  check(expr, pos = null) {
    if (this.dynamic) {
      ...
      if (expr.type === 'FunctionCall' && expr.name.value === 'calldataload') {
        ...
        if (this.nested)
          return Call('__check_calldata_nested', [expr.args[0], pos]);

        return Call('__check_calldata_array', [expr.args[0], unit, pos]);
      }
      ...
      if (expr.type === 'Identifier') {
        ...
        if (this.nested)
          return Call('__check_memory_nested', [expr, pos]);
  ...
  isPadded() {
    if (this.dynamic || this.width === 256)
      return false;
...
class Event {
  ...
  init(node) {
    ...
    for (let i = 0; i < heap.length; i++) {
      ...
      if (param.dynamic && i !== heap.length - 1)
