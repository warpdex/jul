...
class Transformer {
  constructor(root, options, hasher = null) {
    ...
    if (options != null) {
      ...
      assert(typeof wrapping === 'boolean');
    }
  ...
  rewrite(node) {
    ...
    switch (node.type) {
      ...
      case 'ConstructorDefinition': {
        ...
        const check = this.wrapping ? ctor.checkWrap() : ctor.check();
    ...
    switch (name.value) {
      ...
      case 'method.select': {
        ...
        for (const method of scope.methods.values()) {
          ...
          if (this.wrapping) {
            if (method.isView()) {
              signature = method.signature2;
              check = Root([RequireZero(CallValue())]);
            } else {
              check = Root();
            }
          } else {
      ...
      case 'datareference': {
        ...
        if (this.wrapping)
          return Call('datareference', [name]);
...
class ABI {
  ...
  add(object, scope) {
    ...
    if (object.type === 'function' && object.isView())
      this._addSignature(contract, object, object.signature2);
  }
...
class Wrapper {
  ...
  init(root) {
    if (root.type === 'Root') {
      assert(root.nodes.length === 1);
      this.object = root.nodes[0];
    } else {
      this.object = root;
    }

    this.code = this.object.block.body[0];
    this.subobject = this.object.block.body[1];
    this.subcode = this.subobject.block.body[0];

    assert(this.object.type === 'ObjectBlock');
    assert(this.code.type === 'CodeBlock');
    assert(this.subobject.type === 'ObjectBlock');
    assert(this.subcode.type === 'CodeBlock');

    this.collectData(this.object, null, 0);
    this.resolvePaths();
    this.setImmutables();
    this.loadImmutables();
  }

  collectData(object, prefix, depth) {
    ...
    if (depth === 0) {
      ...
      const result = this.compile(object);
      ...
      this.data.set(path, {
        ...
        depth,
        kind: 'bytes',
        data: result.initcode,
        size: result.initcode.length / 2,
        _code: result.bytecode
      });
    } else {
      const parent = this.data.get(prefix);
      ...
      this.data.set(path, {
        ...
        kind: 'bytes',
        data: parent._code,
        size: parent._code.length / 2
      });

      delete parent._code;
    }

    for (const node of object.block.body) {
      switch (node.type) {
        case 'CodeBlock': {
          break;
        }
        case 'ObjectBlock': {
          this.collectData(node, path, depth + 1);
          break;
        }
        case 'DataValue': {
          let kind, data;

          if (node.value.subtype === 'StringLiteral') {
            const str = JSON.parse(node.value.value);
            data = Buffer.from(str, 'utf8').toString('hex');
            kind = 'string';
          } else if (node.value.subtype === 'HexLiteral') {
            data = node.value.value.slice(4, -1);
            kind = 'bytes';
          } else {
            throw new Error('unreachable');
          }
          ...
          this.data.set(`${path}.${node.name}`, item);

          if (node.name === '.metadata' && depth === 1)
            this.metadata = item;
  ...
  resolvePaths() {
    const {object, subobject} = this;

    this._resolvePaths(this.code, object.name);
    this._resolvePaths(this.subcode, `${object.name}.${subobject.name}`);
  }

  _resolvePaths(code, prefix) {
    const copyCalls = new Map();

    for (const node of traverse(code)) {
      if (node.type !== 'FunctionDefinition')
        continue;

      const name = node.name.value;

      if (!name.startsWith('__icreate') && name !== '__construct')
        continue;

      for (const child of node.block.body) {
        if (isDataCopy(child)) {
          copyCalls.set(name, child);
          break;
        }
      }
    }
    ...
    for (const node of traverse(code)) {
      ...
      const copyCall = copyCalls.get(node.name.value);

      if (copyCall) {
        const dataCall = node.args[1];
        const name = JSON.parse(dataCall.args[0].value);
        copyCall.path = `${prefix}.${name}`;
        continue;
      }

      if (isDataCopy(node)) {
        const [, off, size] = node.args;

        let name = null;

        if (isDataOffset(off))
          name = JSON.parse(off.args[0].value);
        else if (isDataSize(size))
          name = JSON.parse(size.args[0].value);
      ...
      if (isDataOffset(node) || isDataSize(node)) {
        const name = JSON.parse(node.args[0].value);
        node.path = `${prefix}.${name}`;
        continue;
      }

      if (isCall(node, 'datareference', 1)) {
        const name = JSON.parse(node.args[0].value);
        node.path = `${prefix}.${name}`;
        continue;
      }
  ...
  setImmutables() {
    const map = this.immutables;

    for (const node of traverse(this.code)) {
      if (node.type === 'FunctionDefinition' &&
          node.name.value === '__construct') {
        this.hasConstruct = true;
        continue;
      }

      if (!isCall(node, 'setimmutable', 3))
        continue;

      const name = JSON.parse(node.args[1].value);
  ...
  loadImmutables() {
    const map = this.dependents;

    for (const func of traverse(this.subcode)) {
      if (func.type !== 'FunctionDefinition')
        continue;

      for (const call of traverse(func.block)) {
        if (!isCall(call, 'loadimmutable', 1))
          continue;

        const name = JSON.parse(call.args[0].value);
        const id = this.immutables.get(name);

        assert(id != null);

        if (!map.has(func.name.value))
          map.set(func.name.value, new Set());

        map.get(func.name.value).add(id);
      }
    }
    ...
    for (;;) {
      ...
      for (const func of traverse(this.subcode)) {
        ...
        for (const call of traverse(func.block)) {
          if (call.type !== 'FunctionCall' || !map.has(call.name.value))
            continue;
          ...
          const names = map.get(func.name.value);

          for (const name of map.get(call.name.value)) {
            updated |= !names.has(name);
            names.add(name);
          }
        }
      }
  ...
  getData(node) {
    if (!node.path)
      throw new Error(`Could not resolve: ${serialize(node)}`);

    const item = this.data.get(node.path);

    if (!item)
      throw new Error(`No data for "${node.path}"`);

    return item;
  }

  wrap() {
    const contract = this.abi.contracts[this.object.name];
    const refs = [];

    // Magic variables (see libsolidity/analysis/GlobalContext.cpp).
    const magic = new Set([
      'abi',
      'assert',
      'block',
    ...
    // Rewrite constructor.
    const initcode = rewrite(this.code.block, (node) => {
      if (node.type === 'Identifier' && magic.has(node.value)) {
        node.value += '$$';
        return node;
      }

      if (node.type === 'Identifier' && node.value.includes('.')) {
        node.value = node.value.replace(/\./g, '$$$$');
        return node;
      }

      if (isDataCopy(node)) {
        const item = this.getData(node);

        if (item.type === 'object' && (item.depth & 1) === 1)
          return Null();

        return copyCode(node.args[0], item.data);
      }

      if (isDataOffset(node)) {
        this.getData(node);
        return Literal(0);
      }

      if (isDataSize(node)) {
        const {size} = this.getData(node);
        return Literal(size);
      }

      if (node.type === 'FunctionDefinition' &&
          node.name.value === '__construct') {
        node.params.items.length = 0;

        for (let i = 0; i < this.immutables.size; i++)
          node.returns.items.push([Identifier(`__imm_ret_${i}`), null]);

        return node;
      }

      if (isCall(node, '__construct', 2)) {
        const rets = [];

        node.args.length = 0;

        for (let i = 0; i < this.immutables.size; i++)
          rets.push(Identifier(`__imm_stack_${i}`));

        return Assignment(IdentifierList(rets), node);
      }

      if (isCall(node, 'setimmutable', 3)) {
        const name = JSON.parse(node.args[1].value);
        const id = this.immutables.get(name);
        const expr = node.args[2];

        if (this.hasConstruct)
          return Assign(Identifier(`__imm_ret_${id}`), expr);

        return Assign(Identifier(`__imm_stack_${id}`), expr);
      }

      if (isCall(node, 'return', 2))
        return MemStoreN(64, Literal(128 + this.immutables.size * 32));

      if (isPopCodeSize(node))
        return Null();

      return node;
    });

    const subcode = rewrite(this.subcode, (node) => {
      const funcs = this.dependents;
      ...
      if (node.type === 'FunctionDefinition' && funcs.has(node.name.value)) {
        for (const id of funcs.get(node.name.value))
          node.params.items.push([Identifier(`__imm_arg_${id}`), null]);
        return node;
      }

      if (node.type === 'FunctionCall' && funcs.has(node.name.value)) {
        for (const id of funcs.get(node.name.value))
          node.args.push(Identifier(`__loadimmutable_${id}`));
        return node;
      }

      if (isCall(node, 'loadimmutable', 1)) {
        const name = JSON.parse(node.args[0].value);
        const id = this.immutables.get(name);
        return Identifier(`__loadimmutable_${id}`);
      }
    ...
    for (const node of traverse(subcode.block)) {
      switch (node.type) {
        case 'FunctionDefinition': {
          inFunction++;
          break;
        ...
        case 'Identifier': {
          ...
          if (!name.startsWith('__loadimmutable_'))
            continue;

          const id = name.substring(16);

          if (inFunction > 1)
            throw new Error('Immutable loaded in nested function.');

          if (inFunction)
            node.value = `__imm_arg_${id}`;
          else
            node.value = `__imm_stack_${id}`;
    ...
    out += `// SPDX-License-Identifier: ${this.abi.license}\n`;
    out += '\n';
    out += `pragma solidity ${this.abi.version};\n`;
    out += '\n';

    if (contract)
      out += eol(contract.comment);

    out += `contract ${this.object.name} {\n`;

    for (const id of this.immutables.values())
      out += `  uint256 private immutable __imm_sol_${id};\n`;

    {
      let str = 'constructor() payable {\n';

      for (const id of this.immutables.values())
        str += `  uint256 __imm_stack_${id};\n`;

      str += indent('assembly ' + serialize(initcode), 2) + '\n';

      for (const id of this.immutables.values())
        str += `  __imm_sol_${id} = __imm_stack_${id};\n`;
    ...
    if (contract) {
      ...
      for (const item of this.abi._sort(contract.items)) {
        if (item.type === 'constructor')
          continue;

        if (item.type === 'event' && item.packed)
          continue;

        let str = item.toSolidity(true);
        ...
        if (item.type === 'function') {
          ...
          if (item.isView()) {
            str += `  __call_view(${item.signature2});\n`;
            hasView = true;
          } else {
            str += '  __implementation();\n';
          }
      ...
      if (!hasFallback && hasView) {
        out += '\n';
        out += '  fallback() external {\n';
        out += '    __implementation();\n';
        out += '  }\n';
      }

      if (hasView) {
        const call = 'staticcall(gas(), address(), 0, calldatasize(), 0, 0)';

        out += '\n';
        out += '  function __call_view(uint32 __sig) private view {\n';
        out += '    assembly {\n';
        out += '      mstore(0, shl(224, __sig))\n';
        out += '      calldatacopy(4, 4, sub(calldatasize(), 4))\n';
        out += `      if iszero(${call}) {\n`;
        out += '        returndatacopy(0, 0, returndatasize())\n';
        out += '        revert(0, returndatasize())\n';
        out += '      }\n';
        out += '      returndatacopy(0, 0, returndatasize())\n';
        out += '      return(0, returndatasize())\n';
        out += '    }\n';
        out += '  }\n';
      }
    } else {
      out += '\n';
      out += '  fallback() external payable {\n';
      out += '    __implementation();\n';
      out += '  }\n';
    }

    out += '\n';
    out += '  function __implementation() private {\n';

    for (const id of this.immutables.values())
      out += `    uint256 __imm_stack_${id} = __imm_sol_${id};\n`;

    out += indent('assembly ' + serialize(subcode.block), 4) + '\n';
    out += '  }\n';

    if (this.metadata)
      refs.push(this.metadata);

    // XXX These could just be comments.
    for (const item of refs) {
      ...
      if (item.type !== 'data')
        throw new Error('Cannot reference objects.');

      if (!/^\w+$/.test(name))
        throw new Error('Invalid datareference name.');

      if (contract) {
        const object = {
          type: 'function',
          name,
          signature: methodHash(name, [])
        };

        this.abi._addName(contract, object);
        this.abi._addSignature(contract, object, object.signature);
      }

      const block = Block([
        MemStoreN(0, Literal(32)),
        MemStoreN(32, Literal(item.size)),
        copyCode(Literal(64), item.data),
        Return(Literal(0), Literal(64 + item.size))
      ]);

      out += '\n';
      out += `  function ${name}()\n`;
      out += '    external pure\n';
      out += `    returns (${item.kind} memory)\n`;
      out += '  {\n';
      out += indent('assembly ' + serialize(block), 4) + '\n';
      out += '  }\n';
    }
...
function wrap(root, abi, compile) {
  return new Wrapper(root, abi, compile).wrap();
}
...
class Constructor {
  ...
  estimateSize() {
    ...
    for (const param of this.params) {
      ...
      if (param.array)
        throw new Error('Cannot have array arguments in wrapped mode.');
  ...
  checkWrap() {
    ...
    const objectSize = Sub(CodeSize(), size);
    const nodes = [];

    if (this.modifier === 'unchecked') {
      if (this.params.length > 0)
        nodes.push(CodeCopy(Literal(0), objectSize, size));
...
class Method {
  ...
  init(node) {
    ...
    const inv = BigInt(this.signature) ^ 0xffffffffn;

    this.signature2 = '0x' + inv.toString(16).padStart(8, '0');
  }
  ...
  toSolidity(forConvert = false) {
    ...
    if (forConvert && this.mutability === 'pure')
      str += ' view';
    else if (this.mutability)
      str += ' ' + this.mutability;
...
function isDataCopy(node) {
  return isCall(node, 'datacopy', 3);
}

function isDataOffset(node) {
  return isCall(node, 'dataoffset', 1) && isStringLiteral(node.args[0]);
}

function isDataSize(node) {
  return isCall(node, 'datasize', 1) && isStringLiteral(node.args[0]);
}

function isPopCodeSize(node) {
  if (!isCall(node, 'verbatim_1i_0o', 2))
    return false;

  if (node.args[0].value !== 'hex"50"')
    return false;

  if (!isCall(node.args[1], 'verbatim_0i_1o', 1))
    return false;

  return node.args[1].args[0].value === 'hex"38"';
}

function copyCode(pos, code) {
  const length = (code.length + 63) & ~63;
  const padded = code.padEnd(length, '0');
  const nodes = [];

  for (let i = 0; i < length; i += 64) {
    const chunk = padded.substring(i, i + 64);
    const num = HexNumber(`0x${chunk}`);

    nodes.push(MemStoreAdd(pos, i / 2, num));
  }
