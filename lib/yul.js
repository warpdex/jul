...
class MemStruct {
  ...
  generate() {
    ...
    for (let i = 0; i < this.members.length; i++) {
      ...
      // Ensure the padding is zeroed.
      if (this.packed) {
        const end = mem.offset + (mem.sizeof() === 1 ? 1 : 32);

        while (written < mem.offset) {
          const pos = Add(obj.name, Literal(written));
          body.push(MemStore(pos, Literal(0)));
          written += 32;
        }
