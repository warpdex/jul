...
class Transformer {
  ...
  builtin(name, node, scope) {
...

    {
      ...
      if (this.debug && this.forceDebug) {
        switch (name.value) {
          case 'assert':
          case 'revert':
            break;
          default:
            // Keep the debug message.
    ...
    switch (name.value) {
      ...
      case 'eth.send': {
        ...
        return EthSend(addr, amt, gas);
      }

      case 'eth.transfer': {
        if (node.args.length < 2 || node.args.length > 4)
          throw new Error(`${name.value}() requires 2-4 arguments.`);
        ...
        switch (node.args.length) {
          case 3: {
            const arg = this.rewrite(node.args[2]);
