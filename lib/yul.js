...
class Parser {
  ...
  FunctionCall() {
    ...
    if (!this.peek(')')) {
      do {
        args.push(this.assert(this.NakedStruct() || this.Expression()));
      } while (this.consume(','));
    }
...
class Scope {
  ...
  *scopes() {
  ...
  findFunc2(name) {
    for (const scope of this.scopes()) {
      const result = scope.funcs2.get(name);
  ...
  findVarType(name) {
    const obj = this.findVar(name);

    if (obj && obj.type != null)
      return Identifier(obj.type);
  ...
  findMember(node) {
    ...
    if (node.cast) {
      obj = {
        name: node.name,
        type: node.cast.value,
        storage: obj ? obj.storage : STACK
      };
    }
    ...
    if (obj.type == null)
      throw new Error(`Referencing undefined type: ${name}`);

    const struct = this.findStruct(obj.type);

    if (struct == null)
      throw new Error(`${name}:${obj.type} is not a struct`);
    ...
    for (const node of block.body) {
      switch (node.type) {
        case 'FunctionDefinition': {
          ...
          for (const [ident, kind, storage] of node.params.items) {
            const name = this.rewrite(ident);
            const type = kind ? this.rewrite(kind) : null;

            params.push([name, type, storage]);
          }

          scope.funcs2.set(name, params);
          ...
          switch (parent.type) {
            ...
            case 'Assignment': {
              if (parent.lhs.items.length === 1) {
                ...
                nodeName = scope.findVarType(name);
              }
              break;
            }
            ...
            case 'FunctionCall': {
              ...
              const params = scope.findFunc2(name);
              if (params && index < params.length)
                nodeName = params[index][1];
              break;
            }
          }
        }
...
class MemStructMember {
  constructor(name, abi, index, offset, packed, default_) {
